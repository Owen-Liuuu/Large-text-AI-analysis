...
def generate_docx_report(
    ...
    doc.add_paragraph(
        ...
        "AI-extracted values from each LLM. Green = consistent, "
        "Red = discrepancy, Grey = not available."
    )
    ...
    if result.comparison_results:
        for cr in result.comparison_results:
            ...
            # Columns: Field | Student | <LLM-A> | <LLM-B> ... | Status
            ...
            # Build a lookup: extractor_id → {field_name: value}
            # Normalise field names to allow partial matching
            ai_lookup: dict[str, dict[str, object]] = {}
            for et in result.extracted_tables:
                # Match this extracted table to the comparison result's paper
                et_norm = (et.paper_id or "").strip().lower()
                cr_norm = (cr.paper_id or "").strip().lower()
                for prefix in ("https://doi.org/", "http://doi.org/", "doi:"):
                    et_norm = et_norm.removeprefix(prefix)
                    cr_norm = cr_norm.removeprefix(prefix)
                if et_norm != cr_norm:
                    continue
                field_map = {f.field_name: f.value for f in et.fields}
                ai_lookup[et.extractor_id] = field_map

            for i, diff in enumerate(cr.field_diffs, 1):
                _set_cell(tbl, i, 0, diff.field_name)

                # Student value
                s_val = _fmt(diff.student_value)
                _set_cell(tbl, i, 1, s_val)

                # AI values — one column per extractor (always rendered)
                for j, eid in enumerate(extractor_ids):
                    fmap = ai_lookup.get(eid, {})
                    # Try exact field name first, then model_values[j]
                    if diff.field_name in fmap:
                        ai_val = _fmt(fmap[diff.field_name])
                    elif j < len(diff.model_values):
                        ai_val = _fmt(diff.model_values[j])
                    else:
                        ai_val = "—"
                    cell = tbl.cell(i, 2 + j)
                    cell.text = ai_val

                # Status column — render from the rich FieldStatus enum so
                # extractor gaps are visually distinct from real diffs.
